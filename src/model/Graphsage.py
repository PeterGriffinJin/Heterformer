...
class GraphSageForNeighborPredict(BertPreTrainedModel):
    ...
    def test(self, input_ids_query_and_neighbors_batch, attention_mask_query_and_neighbors_batch, 
        ...
        tweet_neighbour_c = input_ids_query_and_neighbors_batch.shape[1]

        query_embeddings = self.infer(input_ids_query_and_neighbors_batch, attention_mask_query_and_neighbors_batch,
                                        query_mta_neighbors_batch, mask_query_and_neighbors_batch[:,:tweet_neighbour_c])
        key_embeddings = self.infer(input_ids_key_and_neighbors_batch, attention_mask_key_and_neighbors_batch,
                                    key_mta_neighbors_batch, mask_key_and_neighbors_batch[:,:tweet_neighbour_c])
...
class GraphMAXSageForNeighborPredict(GraphSageForNeighborPredict):
    def __init__(self, config):
        super().__init__(config)
...
class GraphMeanSageForNeighborPredict(GraphSageForNeighborPredict):
    def __init__(self, config):
        super().__init__(config)
