...
def parse_args():
    ...
    parser.add_argument('--method', type=str, default='BERT', choices=['BERT', 'GAT', 'Graphformer', 'GraphMaxSage', 
                                                'GraphMeanSage', 'GraphformerAbl', 'Heterformer', 'HeterformerD', 'HeterformerDT', 
...
def main(args):
    # read file
    embedding = np.load(os.path.join(args.dataset+'_embed', args.dataset+'_embed', args.method+'_'+args.mode+'.npy'))
    labels = np.load(os.path.join(args.dataset+'_embed', args.dataset+'_embed', args.mode+'_label.npy'))
    ...
    best_acc = 0 
    best_count = 0
    ckpt_path = os.path.join('rebuttal', args.dataset+'_embed', '{}-{}-{}-{}-best.pt'.format(args.method, args.mode, args.lr, args.hidden_size))
